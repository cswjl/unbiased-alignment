...
from trl.trainer.utils import disable_dropout_in_model, pad, remove_none_values
    ...
    def compute_loss(
        ...
        if "margin" in inputs:
            ...
            if self.args.loss_type == 'sigmoid':
                loss = (
                    - nn.functional.logsigmoid(rewards_chosen - rewards_rejected) * (1 - self.args.label_smoothing)
                    - nn.functional.logsigmoid(rewards_rejected - rewards_chosen) * self.args.label_smoothing
                )
                loss = loss.mean()
                ...
                loss = (
                    - nn.functional.logsigmoid(rewards_chosen - rewards_rejected) * (1 - self.args.label_smoothing)
                    + nn.functional.logsigmoid(rewards_rejected - rewards_chosen) * self.args.label_smoothing
                ) / (1 - 2 * self.args.label_smoothing)
                loss = loss.mean()

            elif self.args.loss_type == 'unbiased':

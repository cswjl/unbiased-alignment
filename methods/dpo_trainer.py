...
class DPOTrainer(BaseTrainer):
    ...
    def dpo_loss(
        ...
        if loss_type == "sigmoid":
            ...
            prob = (torch.exp(self.beta * logits) + self.args.unbiased_a) / (torch.exp(self.beta * logits) + 1)
            losses = - torch.log(prob) * normal

        elif loss_type == "ropo":
            l_na = torch.sigmoid( - self.beta * logits)
            l_dpo = - F.logsigmoid(self.beta * logits)
            losses = (
                (4 * self.args.ropo_alpha**2) / (1 + self.args.ropo_alpha)**2 * l_na
                +  (4 * self.args.ropo_alpha) / (1 + self.args.ropo_alpha)**2 * l_dpo
            )
        elif loss_type == "drdpo":
            l_dpo = - F.logsigmoid(self.beta * logits)
            losses = - self.args.drdpo_beta * torch.log(torch.mean(torch.exp( - l_dpo / self.args.drdpo_beta)))
        else:
            raise ValueError(
